...
int main(void) {
    ...
    printf("\n\nWelcome to ktruon13's sort.c, compiled on %s %s.\n", __DATE__, __TIME__);
...
ListItem *SelectionSort(ListItem* list) {
    ...
    while (first->nextItem != NULL) {
        ListItem *second = first->nextItem;
        while (second != NULL) {
            if (strcmp(first->data, second->data) > 0) {
                LinkedListSwapData(first, second);
            }
            second = second->nextItem;
        }
...
ListItem *InsertionSort(ListItem* list) {
    ...
    while (first->previousItem != NULL) {
        ListItem *second = first->previousItem;
        if (strcmp(second->data, first->data) < 0) {
            first = second;
        } else {
            sort = first;
            while (sort->nextItem != NULL) {
                if (strcmp(sort->nextItem->data, second->data) > 0) {
                    break;
                } else {
                    sort = sort->nextItem;
                }
            }
            char *data = LinkedListRemove(second);
            LinkedListCreateAfter(sort, data);
        }
    }
    return LinkedListGetFirst(list);
}
